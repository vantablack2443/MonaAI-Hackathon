...
machine's main memory size. In this milestone, you will remove that
limitation.

You will build this milestone on top of the last one. Test programs
from milestone 2 should also work with milestone 3. You should take care to
fix any bugs in your milestone 2 submission before you start work on
milestone 3, because those bugs will most likely cause the same problems
in milestone 3. Alternatively, you can build on the reference solution we
will release after the last slip days have passed.

You will continue to handle Pintos disks and file systems the same way
you did in the previous milestone (see section [Using the File
...
You will work in the `vm` directory for this milestone. The `vm` directory
contains only `Makefile`s. The only change from `userprog` is that this
new `Makefile` turns on the setting `-DVM`. All code you
write will be in new files or in files introduced in earlier milestones.
...
- `devices/block.c`: Provides sector-based read and write access to
                     a block device. You will use this interface to access the
...
memory and storage. Some of these terms should be familiar from milestone
...
[Virtual Addresses](../../doc/reference_guide.md#virtual-addresses),
for details.
...
page-sized and page-aligned. Thus, a 32-bit physical address can be
...
(see section [Page Table](../../doc/reference_guide.md#page-table)).
...
- **Supplemental page table**: Enables page fault handling by
  supplementing the hardware page table.
  See section
...
(see section [Hash Table](../../doc/reference_guide.md#hash-table)).
...
[Page Table Entry Format](../../doc/reference_guide.md#page-table-entry-format),
for more information.

The most important user of the supplemental page table is the page fault
handler. In milestone 2, a page fault always indicated a bug in the kernel
or a user program. In milestone 3, this is no longer true. Now, a page
...
1. Choose a frame to evict, using your page replacement algorithm. The accessed*
and *dirty* bits in the page table, described below, will come in handy.

2. Remove references to the frame from any page table that refers to it.
Unless you have implemented sharing, only a single page should refer to a frame
at any given time.
...
See section
[Accessed and Dirty Bits](../../doc/reference_guide.md#accessed-and-dirty-bits),
...
`pintos-mkdisk swap.dsk --swap-size=n` to create a disk named
...
1.  Frame table (see section
    ...
    After this step, your kernel should still pass all the milestone 2
    test cases.
...
2.  Supplemental page table and page fault handler (see section
    ...
    After this step, your kernel should pass all of the milestone 2
...
Finally, implement an eviction strategy such as the clock algorithm.


## Requirements

This milestone is an open-ended design problem. We are going to say as
...
You are welcome to review your design with teaching staff during office
hours to identify potential problems before you fully implement it.


### Design Document

Before you turn in your milestone, you must fill in the milestone 3 design
document template under `src/vm/DESIGNDOC.md`. We recommend that you read the
design document template before you start working on the milestone. See
section [Milestone Documentation](../../doc/milestone_doc.md),
for a sample design document that goes along with a fictitious milestone.
...
Implement stack growth. In milestone 2, the stack was a single page at the
...
Depending on code style and design choices, typical implementations
here range from 1000-2000 lines of code added and modified in Pintos.

**Do we need a working milestone 2 to implement milestone 3?**
...
section
[Internal Interrupt Handling](../../doc/reference_guide.md#internal-interrupt-handling)).
...
the allocated space that is `m` bytes above the current
...
Also, you can use the `-ul` kernel command-line option to
