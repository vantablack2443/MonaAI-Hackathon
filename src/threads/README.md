...
- [FAQ](#faq)
  ...
  - [Priority Scheduling FAQ](#priority-scheduling-faq)
    ...
    - [What happens to the priority of a thread while it is the benefactor of an inheriting thread?](#what-happens-to-the-priority-of-a-thread-while-it-is-the-benefactor-of-an-inheriting-thread)
    ...
    - [How does `thread_set_priority()` affect a thread inheriting priorities?](#how-does-thread_set_priority-affect-a-thread-inheriting-priorities)
...
work in the `src/devices` directory on the side. Compilation should be done in
`src/threads`.
...
2. [Coding Standards](../../doc/coding_standards.md)
3. [Debugging Tools](../../doc/debugging_tools.md)
4. In the [Reference Guide](../../doc/reference_guide.md), you should at least
    skim the sections on [Loading](../../doc/reference_guide.md#loading)
    through
    [Memory Allocation](../../doc/reference_guide.md#memory-allocation),
    especially
    [Synchronization](../../doc/reference_guide.md#synchronization).
...
introduction](../../doc/introduction.md), you should do so now. You can read
...
(see [vscode Debugger](../../doc/debugging_tools.md#vscode) and
[GDB](../../doc/debugging_tools.md#gdb)). You can set a breakpoint on
...
Switching](../../doc/reference_guide.md#thread-switching), for more
information.
...
Allocation](../../doc/reference_guide.md#memory-allocation)).
...
* `loader.S`, `loader.h`: The kernel loader. Assembles to 512 bytes of code and
    ...
    Loader](../../doc/reference_guide.md#the-loader), for details. You should
...
* `start.S`: Does basic setup needed for memory protection and 32-bit operation
    ...
    Initialization](../../doc/reference_guide.md#low-level-kernel-initialization),
    for details.
* `kernel.lds.S`: The linker script used to link the kernel. Sets the load
    ...
    Loader](../../doc/reference_guide.md#the-loader), for details. Again, you
...
* `init.c`, `init.h`: Kernel initialization, including `main()`, the kernel's
    ...
    [High-Level Kernel Initialization](../../doc/reference_guide.md#high-level-kernel-initialization), for details.
* `thread.c`, `thread.h`: Basic thread support. Much of your work will take
    ...
    thread`](../../doc/reference_guide.md#struct-thread) and
    [Threads](../../doc/reference_guide.md#threads) for more information.
* `switch.S`, `switch.h`: Assembly language routine for switching threads.
    Already discussed above. See
    [Thread Functions](../../doc/reference_guide.md#thread-functions), for
    more information.
* `palloc.c`, `palloc.h`: Page allocator, which hands out system memory in
    multiples of 4 kB pages. See
    [Page Allocator](../../doc/reference_guide.md#page-allocator), for
    more information.
* `malloc.c`, `malloc.h`: A simple implementation of `malloc()` and `free()` for
    the kernel. See
    [Block Allocator](../../doc/reference_guide.md#block-allocator), for more
    information.
* `interrupt.c`, `interrupt.h`: Basic interrupt handling and functions for
    turning interrupts on and off. See
    [Interrupt Handling](../../doc/reference_guide.md#interrupt-handling), for
    more information.
* `intr-stubs.S`, `intr-stubs.h`: Assembly code for low-level interrupt
    handling. See section [Interrupt Infrastructure](../../doc/reference_guide.md#interrupt-infrastructure), for more information.
* `synch.c`, `synch.h`: Basic synchronization primitives: semaphores, locks,
    ...
    [Synchronization](../../doc/reference_guide.md#synchronization), for more
    information.
...
* `flags.h`: Macros that define a few bits in the x86 "flags" register. Probably
    of no interest. See [IA32-v1](../../doc/bibliography.md#IA32-v1),
...
* `string.h`: A subset of the standard C library. See section
    [C99](../../doc/coding_standards.md#c99), for information on a few
    recently introduced pieces of the C library that you might not have
    encountered before. See section
    [Unsafe String Functions](../../doc/coding_standards.md#unsafe-string-functions),
...
* `debug.c`, `debug.h`: Functions and macros to aid debugging. See section
    [Debugging Tools](../../doc/debugging_tools.md), for more information.
...
synchronization (see
[Synchronization](../../doc/reference_guide.md#synchronization)) or the
...
In the past, some students divided the assignment into pieces, working on each piece
separately until just before the deadline, at which time
they tried to combine all their code and submit. **This is a bad idea. We
do not recommend this approach.** Students that do this often find that two
changes conflict with each other, requiring lots of last-minute debugging. Some
students who have done this have turned in code that did not even compile or boot,
much less pass any tests.

Instead, we recommend integrating your changes early and often, using `git`.
Proper use of a version-control system (VCS) like `git` also makes it possible
to review changes and, when a change introduces a bug, drop back to working
versions of code. For example, `git-bisect` can be extremely useful to track
down when a particular bug was introduced, but it is effective only if you
commit your changes frequently and as topically meaningful change sets. 

You should expect to run into bugs that you simply don't yet understand while
working on this and subsequent projects. That is expected --- the projects are
an integral part of this course's learning experience. When you do, reread the
documentation on debugging tools, which is filled with useful debugging tips
that should help you to get back up to speed (see [Debugging
Tools](../../doc/debugging_tools.md)). Be sure to read the section on
backtraces (see [Backtraces](../../doc/debugging_tools.md#backtraces)), which
will help you to get the most out of every kernel panic or assertion failure.

Finally, when stuck, do approach the TAs for help in due time _before_ the
submission deadline.
...
on the project. See [Project Documentation](../../doc/project_doc.md),
...
[Debugging versus Testing](../../doc/introduction.md#debugging-versus-testing)).
...
different interleavings of characters are possible. However, note that a correct
...
One issue with priority scheduling is [priority
inversion](https://en.wikipedia.org/wiki/Priority_inversion). Consider high,
...
get any CPU time. A partial fix for this problem is for `L` to _inherit_ `H`'s
priority while `L` is holding the lock that `H` needs; the "inherited" priority
is returned once `L` releases (and thus `H` acquires) the lock.

Implement priority inheritance. You will need to account for all different
situations in which priority inheritance is required. Be sure to handle multiple
inheritance, in which multiple priorities are inherited by a single thread. You
must also handle _transitive_ priority inheritance: if `H` is waiting on a lock
that `M` holds and `M` is waiting on a lock that `L` holds, then both `M` and
`L` should be boosted to `H`'s priority. If necessary, you may impose a
reasonable limit on the supported "depth" of transitive priority inheritance,
such as 8 levels.

You must implement priority inheritance for locks. You need not implement
priority inheritance for the other Pintos synchronization constructs. You do
need to implement priority scheduling in all cases.
...
```
> Returns the current thread's priority. In the presence of priority inheritance,
> returns the higher (inherited) priority.
...
See [Testing](../../doc/introduction.md#testing).
...
Attributes](../../doc/debugging_tools.md#function-and-parameter-attributes)),
...
See [Backtraces](../../doc/debugging_tools.md#backtraces), for more
information.
...
#### What happens to the priority of a thread while it is the benefactor of an inheriting thread?

Priority inheritance only changes the effective priority of the inheriting
thread. The priority of the giving thread remains unchanged. 

Priority inheritance is not additive: if thread `B` (with priority 3) inherits
from thread `A` (with priority 5), then `B`'s new effective priority is 5, not
8.

#### Can a thread's priority change while it is on the ready queue?

Yes. Consider a ready, low-priority thread `L` that holds a lock/. Suppose a
high-priority thread `H` attempts to acquire the lock and blocks, thereby
letting the ready thread `L` inherit its priority.
...
priority can increase via priority inheritance if a higher-priority thread
attempts to acquire `L`. This case is checked by the `priority-donate-sema`
test.
...
#### How does `thread_set_priority()` affect a thread inheriting priorities?

It sets the thread's base priority. The thread's effective priority becomes the
higher of the newly set priority or the highest inherited priority. When
priority inheritance ceases, the thread's priority becomes the one set through
the function call. This behavior is checked by the `priority-donate-lower` test.
