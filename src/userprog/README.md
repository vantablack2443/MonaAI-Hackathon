...
interactivity is possible. In this milestone, you will enable programs to
...
You can include code from your milestone 1 submission, but no code from
milestone 1 is required for this assignment. The "alarm clock" functionality may
be useful in milestones 3 and 4, but it is not strictly required. We will
provide a reference solution for `ms2` after the deadline + 4 days (to account for
slip days), if you prefer to build on this instead of your own `ms2` solution.

You might find it useful to go back and reread how to run the tests (see
section [Testing](../../doc/introduction.md#testing)).
...
milestone deals with the consequences.
...
In the previous milestone, we compiled our test code directly into your
...
* `pagedir.c`, `pagedir.h`:  A simple manager for 80x86 hardware page tables.
    Although you probably won't want to modify this code for this milestone, you
    may want to call some of its functions. See
    [Page Tables](../../doc/reference_guide.md#page-table), for more
    information.
* `syscall.c`, `syscall.h`: Whenever a user process wants to access some kernel
    ...
    process. In part 2 of this milestone you will add code to do everything else
...
* `exception.c`, `exception.h`: When a user process performs a privileged or
    ...
    message and terminate the process. Some, but not all, solutions to milestone
    2 require modifying `page_fault()` in this file.
* `gdt.c`, `gdt.h`: The 80x86 is a segmented architecture. The Global Descriptor
    ...
    milestones.** You can read the code if you're interested in how the GDT works.
* `tss.c`, `tss.h`: The Task-State Segment (TSS) is used for 80x86 architectural
    ...
    modify these files for any of the milestones.** You can read the code if
    you're interested in how the TSS works.

### Using the File System

You will need to interface to the file system code for this milestone,
...
focus of this milestone is not the file system, so we have provided a
...
There is no need to modify the file system code for this milestone, and so
we recommend that you do not. Working on the file system is likely to
distract you from this milestone's focus.

Proper use of the file system routines now will make life much easier
for milestone 4, when you improve the file system implementation. Until
...
until you implement it.) It assumes that you've already built the
...
implemented because none of the system calls required for this milestone
...
programs will only work once milestones 3 or 4 have been implemented.
...
In this milestone, the user stack is fixed in size, but in milestone 3 it
...
specified in [SysV-i386](../../doc/bibliography.md#SysV-i386)
...
Before you turn in your milestone, **you must fill in [the milestone 2 design
...
on the milestone. See section [Project Documentation](../../doc/project_doc.md),
...
new file is a separate operation that would require an `open` system
call.
...
files have a fixed length until milestone 4 is complete, so writes
...
some of them in milestone 3 and the rest in milestone 4, so be sure to
...
We have provided a user-level function for each system call in
...
important once virtual memory is implemented in milestone 3, but it can't
hurt even now.
...
-   The `objdump` (80x86) or `i386-elf-objdump` (SPARC)
    ...
    [GDB](../../doc/debugging_tools.md#gdb)).
...
-   Yes, with some limitations. See
    [Debugging Tools](../../doc/debugging_tools.md).
...
-   You may modify the stack setup code to allocate more than one page
    of stack space for each process. In the next milestone, you will
...
-   `exec` should return -1 if the child process fails to load for any
    ...
    (see section [Semaphores](../../doc/reference_guide.md#semaphores)),
...
details, refer to [SysV-i386](../../doc/bibliography.md#SysV-i386).
...
The first milestone already dealt with one way that the operating system
...
Handling](../../doc/reference_guide.md#external-interrupt-handling)).
